...
DEMO = True  # Change to False only after successful demo testing!
...
# OANDA PRO BOT v3.0 — £100 TEST EDITION

# Optimised for small account — safe sizing, high quality trades

# Target: 15-35% monthly if signals are strong
...
# ── Risk settings optimised for £100 ─────────────────────────

RISK_PER_TRADE_PCT  = 2.0    # Risk 2% per trade = ~£2 per trade
STOP_LOSS_PCT       = 0.010  # 1.0% stop loss
TAKE_PROFIT_PCT     = 0.040  # 4.0% take profit = 4:1 reward ratio
MIN_REWARD_RATIO    = 3.0

# ── Hard account protection ───────────────────────────────────

MAX_DAILY_LOSS_PCT  = 6.0    # Stop if 6% lost today (~£6)
MAX_WEEKLY_LOSS_PCT = 12.0   # Stop if 12% lost this week (~£12)
MAX_DRAWDOWN_PCT    = 20.0   # Emergency stop at 20% drawdown (~£20)
MAX_OPEN_TRADES     = 3
MAX_DAILY_TRADES    = 10
MAX_CONSEC_LOSSES   = 3
MIN_BALANCE         = 20.0   # Never trade below £20
...
def get_balance():
try:
r = requests.get(f”{BASE_URL}/v3/accounts/{OANDA_ACCOUNT_ID}/summary”, headers=HEADERS, timeout=10)
...
log.error(f”Candle error ({instrument}): {e}”)
return None

def get_open_trades():
try:
r = requests.get(f”{BASE_URL}/v3/accounts/{OANDA_ACCOUNT_ID}/openTrades”, headers=HEADERS, timeout=10)
...
try:
r    = requests.post(f”{BASE_URL}/v3/accounts/{OANDA_ACCOUNT_ID}/orders”, headers=HEADERS, json=payload, timeout=10)
resp = r.json()
if “orderFillTransaction” in resp:
fill = resp[“orderFillTransaction”]
log.info(f”✅ {direction} | {instrument} | Fill: {fill.get(‘price’)} | SL: {sl:.{digits}f} | TP: {tp:.{digits}f}”)
return True
...
def main():
log.info(“╔══════════════════════════════════════════════╗”)
log.info(“║   OANDA PRO BOT v3.0 — £100 TEST MODE       ║”)
log.info(f”║   Mode: {‘DEMO ✅’ if DEMO else ‘LIVE 🔴’} | Risk: 2%/trade          ║”)
log.info(“╚══════════════════════════════════════════════╝”)
...
while True:
    try:
        ...
        if daily_loss >= start_bal * MAX_DAILY_LOSS_PCT / 100:
            log.warning(f"🛑 Daily loss limit: £{daily_loss:.2f}")
            time.sleep(SCAN_INTERVAL)
            continue

        if weekly_loss >= week_bal * MAX_WEEKLY_LOSS_PCT / 100:
            log.warning(f"🛑 Weekly loss limit: £{weekly_loss:.2f}")
            time.sleep(SCAN_INTERVAL)
            continue

        if daily_trades >= MAX_DAILY_TRADES:
            log.warning(f"🛑 Daily trade limit: {daily_trades}")
            time.sleep(SCAN_INTERVAL)
            continue
        ...
        for inst in INSTRUMENTS:
            ...
            log.info(f"🎯 {signal} | {label} | {price:.{digits}f} | SL:{sl:.{digits}f} TP:{tp:.{digits}f} RR:{rr:.1f}:1")
        ...
        log.info(f"💰 £{balance:.2f} | Open:{len(open_trades)}/{MAX_OPEN_TRADES} | Trades:{daily_trades}/{MAX_DAILY_TRADES} | P&L:£{pnl:.2f} | DD:{drawdown:.1f}%")
